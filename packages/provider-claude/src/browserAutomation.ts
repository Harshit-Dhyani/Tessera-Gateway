...
export interface ClaudeBrowserAutomationResult {
  ok: boolean;
  text: string;
  errorCode?: ErrorCode;
  errorMessage?: string;
...
export function createClaudePromptScript(prompt: string, timeoutMs = 120000): string {
  return `
(() => {
  ...
  const responseMessages = () => {
    ...
    for (const selector of selectors) {
      for (const element of Array.from(document.querySelectorAll(selector))) {
        ...
        if (text && text !== prompt && !text.toLowerCase().includes('claude can make mistakes')) {
          messages.push(text);
        }
      }
  ...
  return (async () => {
    ...
    if (!composer) {
      return result({
        ...
        errorMessage: pageNeedsLogin()
          ? 'Claude requires manual sign-in before prompt execution.'
          : 'Claude composer was not found. Wait for the page to finish loading or verify the provider UI.',
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (Date.now() - stableSince >= 2000) {
        return result({
          ok: true,
          text: candidate,
          captureMethod: 'claude_response_stable_text:' + submitMethod,
        });
      }
    }
    ...
    return result({
      ...
      errorMessage: 'Timed out waiting for a stable Claude response.',
      captureMethod: 'claude_response_stable_text:' + submitMethod,
    });
  })();
})()
